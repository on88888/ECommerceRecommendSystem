...
object StatisticsRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val rateMoreProductsDF = spark
      .sql("select productId, count(productId) as count " +
    ...
    val ratingOfYearMonthDF = spark.sql("select productId," +
      " score, changeDate(timestamp) as yearmonth from ratings")
    ratingOfYearMonthDF.createOrReplaceTempView("ratingOfMonth")
    val rateMoreRecentlyProductsDF = spark.sql("select " +
      "productId, count(productId) as count, yearmonth from ratingOfMonth " +
    ...
    val averageProductsDF = spark.sql("select productId," +
      " avg(score) as avg from ratings group by productId order by avg desc")
