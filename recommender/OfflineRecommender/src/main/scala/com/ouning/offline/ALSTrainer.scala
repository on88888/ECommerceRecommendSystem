package com.ouning.offline

import breeze.numerics.sqrt
import com.ouning.offline.OfflineRecommender.MONGODB_RATING_COLLECTION
...
object ALSTrainer {
  ...
  def adjustALSParams(trainData: RDD[Rating],
                      testData: RDD[Rating]): Unit ={
    // 遍历数组中定义的参数取值
    val result = for( rank
                        <- Array(5, 10, 20, 50);
                        lambda <- Array(1, 0.1, 0.01) )
      yield {
        val model = ALS
          .train(trainData, rank, 10, lambda)
